...
import { deleteCustomer, getCustomers } from '@/lib/actions/customer.action';
import Cookies from 'js-cookie';

const CustomersTable = () => {
  const [currentCustomers, setCustomers] = useAtom(representativesAtom);
  ...
  const handleDeleteRep = async (id) => {
    ...
    try {
      ...
      setCustomers(currentCustomers.filter((rep) => rep.id !== id));
    } catch (error) {
      console.log(error);
    }
    setLoadingId(null);
  ...
  return (
    <Table>
      ...
      <TableBody>
        {currentCustomers.length > 0 && currentCustomers.map((rep) => (
          <TableRow key={rep.id}>
